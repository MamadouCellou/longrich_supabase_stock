...
class _PurchasesListPageState extends State<PurchasesListPage> {
  ...
  Future<void> _loadPurchases({bool showLoader = false}) async {
    ...
    try {
      ...
      for (var p in _purchases) {
        print(p); // grâce au toString(), ça sera lisible
      }
    } catch (e) {
  ...
  void _subscribeRealtime() {
    _subPurchases = supabase.from('purchases').stream(
        primaryKey: ['id']).listen((_) => _loadPurchases(showLoader: false));

    _subItems = supabase.from('purchase_items').stream(primaryKey: [
      'id'
                      ...
                      child: ExpansionTile(
                        ...
                        trailing: Column(
                          ...
                          children: [
                            ...
                            Text(
                                "PV: ${_currencyFormat.format(purchase.totalPv)}"),
                          ],
                        ),
                        children: [
                            ...
                            children: [
                              PopupMenuButton<String>(
                                ...
                                onSelected: (value) async {
                                  if (value == 'edit') {
                                    final itemsRes =
                                        await _loadItems(purchase.id!);
                                    print(
                                        "Ici la commande ${purchase.toMap()}");
                                    Navigator.push(
                                      context,
                                      MaterialPageRoute(
